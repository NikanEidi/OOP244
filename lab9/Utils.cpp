...
namespace seneca {
   ...
   char* Utils::alocpy(char*& des, const char* src) const {
    if (des == src) { // Avoid self-assignment
        return des;
    }
    if (des) { // Check if des is pointing to allocated memory
        delete[] des; // Free any existing memory
        des = nullptr; // Reset pointer to avoid dangling pointers
    }
    if (src) {
        des = new char[strlen(src) + 1]; // Allocate memory for the new string
